#!/usr/bin/env python3
"""
Health check server for general agent container.
...
class HealthCheckHandler(BaseHTTPRequestHandler):
    ...
    def _get_basic_health(self) -> Dict[str, Any]:
        ...
        critical_services = ['xserver', 'vnc', 'novnc', 'jupyter']
    ...
    def _check_services(self) -> Dict[str, str]:
        ...
        try:
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.settimeout(2)
            jupyter_running = sock.connect_ex(('localhost', 8888)) == 0
            sock.close()
            services['jupyter'] = 'healthy' if jupyter_running else 'unhealthy'
        except:
            services['jupyter'] = 'unknown'
